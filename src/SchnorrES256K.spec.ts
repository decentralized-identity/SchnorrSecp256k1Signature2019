...
const expectedSignatureCompact = 'eyJhbGciOiJTY2hub3JyRVMyNTZLIn0.eyJoZWxsbyI6dHJ1ZX0.6OdF-pxSidTAcTldIhy7bVkxPzM_vhgKw-cjqhvlcfBCl_xhUMLY8uDNlwGVLg0CC5tXCW12oLbb2LV2V-26rw'
...
describe('SchnorrES256K', () => {
  ...
  describe('verify', () => {
    it('should return the decoded payload for a valid SchnorrES256K', async () => {
        const verified = await SchnorrES256K.verify(expectedSignatureCompact, publicKeyJwk);
  ...
  describe('decode', () => {
    ...
    it('should return the decoded complete payload for a SchnorrES256K', async () => {
      const decoded = await SchnorrES256K.decode(expectedSignatureCompact, { complete: true });
